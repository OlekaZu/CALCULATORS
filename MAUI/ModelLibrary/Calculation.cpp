...
namespace s21 {
    ...
    std::string Calculation::GetError() { return error_; }

    std::string Calculation::GetInput() { return input_;}

    void Calculation::Calculate(const std::string& input, double x_argument) {
        input_ = input;
        if (!input_.empty()) {
            CheckBeforeParsing();
            if (error_.empty()) {
    ...
    void Calculation::CheckBeforeParsing() {
        int amount_left_bracket = 0, amount_right_bracket = 0;
        for (size_t i = 0; i < input_.length(); ++i) {
            if (CheckCorrectSymbols(input_[i]) ||
                CheckPointPosition(input_, i) ||
                CheckPointNumber(input_, i) ||
                CheckRepeatOperators(input_, i) ||
    ...
    bool Calculation::CheckCorrectSymbols(const char& symbol) {
        return (operators.find(symbol) == std::string::npos &&
            numbers.find(symbol) == std::string::npos &&
            letters.find(symbol) == std::string::npos);
    }

    bool Calculation::CheckPointPosition(std::string input, size_t i) {
        return (input[i] == 46 &&
            (!i || !isdigit(input[i - 1]) || !isdigit(input[i + 1])));
    }

    bool Calculation::CheckPointNumber(std::string input, size_t i) {
    ...
    bool Calculation::CheckRepeatOperators(std::string input, size_t i) {
        return (operators_2.find(input[i]) != std::string::npos &&
            operators_2.find(input[i + 1]) != std::string::npos);
    }

    bool Calculation::CheckLeftBracket(std::string input, size_t i) {
    ...
    bool Calculation::CheckRightBracket(std::string input, size_t i, size_t left) {
    ...
    bool Calculation::CheckExponentFormat(std::string input, size_t i) {
        return (input[i] == 'e' && ((input[i + 1] != '-' && input[i + 1] != '+') ||
            (i && !isdigit(input[i - 1]))));
    }

    bool Calculation::CheckBracketsNumber(size_t right, size_t left) {
        return (right != left);
    }

    bool Calculation::CheckLastElement(std::string input) {
    ...
    bool Calculation::CheckFirstElement(std::string input) {
        const char& first_element = input.front();
        return (first_element == 'e' || first_element == '*' || first_element == '/'
            || first_element == '^');
    }
    ...
    void Calculation::ParceSymbolOperator(std::string input, size_t i) {
    ...
    void Calculation::ParceNumber(std::string input, size_t* i) {
        size_t last_index = input.size() - 1;
    ...
    void Calculation::ParceArgumentX(std::string input, size_t i) {
    ...
    void Calculation::ParceArgumentPi(std::string input, size_t i) {
        std::string char_in_str(1, input[i]);
        if ((operators.find(input[i + 1]) == std::string::npos &&
            input[i + 1] != 'm' && i != input.size() - 1) ||
            (i > 0 && operators.find(input[i - 1]) == std::string::npos &&
    ...
    void Calculation::ParceAcosAsinAtan(std::string input, size_t* i) {
    ...
    void Calculation::ParceCos(std::string input, size_t* i) {
        std::string cos = "cos(";
    ...
    void Calculation::ParceLnLog(std::string input, size_t* i) {
        std::string ln = "ln(";
        std::string log = "log(";
    ...
    void Calculation::ParceMod(std::string input, size_t* i) {
        std::string mod = "mod";
    ...
    void Calculation::ParceSinSqrt(std::string input, size_t* i) {
        std::string sin = "sin(";
    ...
    void Calculation::ParceTan(std::string input, size_t* i) {
        std::string tan = "tan(";
    ...
    void Calculation::CalculateResult(double x_argument) {
    ...
    void Calculation::calc_inversion() {
        double num_1 = top_and_pop();
        double result = num_1 * (-1);
        save_result_.push(result);
    }

    void Calculation::calc_acos() {
        double num_1 = top_and_pop();
        double result = acos(num_1);
        save_result_.push(result);
    }

    void Calculation::calc_asin() {
        double num_1 = top_and_pop();
        double result = asin(num_1);
        save_result_.push(result);
    }

    void Calculation::calc_atan() {
        double num_1 = top_and_pop();
        double result = atan(num_1);
        save_result_.push(result);
    }

    void Calculation::calc_cos() {
        double num_1 = top_and_pop();
        double result = cos(num_1);
        save_result_.push(result);
    }

    void Calculation::calc_ln() {
        double num_1 = top_and_pop();
        double result = log(num_1);
        save_result_.push(result);
    }

    void Calculation::calc_log() {
        double num_1 = top_and_pop();
        double result = log10(num_1);
        save_result_.push(result);
    }

    void Calculation::calc_mod() {
        double num_2 = top_and_pop();
        double num_1 = top_and_pop();
    ...
    void Calculation::calc_sin() {
        double num_1 = top_and_pop();
        double result = sin(num_1);
        save_result_.push(result);
    }

    void Calculation::calc_sqrt() {
        double num_1 = top_and_pop();
        double result = sqrt(num_1);
        save_result_.push(result);
    }

    void Calculation::calc_tan() {
        double num_1 = top_and_pop();
    ...
    double Calculation::top_and_pop() {
...
s21::Calculation * CreateCalculationClass() {
    return new s21::Calculation();
}

void DisposeCalculationClass(s21::Calculation * ptr_object) {
    if (ptr_object != NULL) {
        delete ptr_object;
        ptr_object = NULL;
    }
...
void CallCalculate(s21::Calculation * ptr_object, const char* input, double x_argument) {
    if (ptr_object != NULL) {
        ptr_object->Calculate(input, x_argument);
    }
}

double CallGetData(s21::Calculation * ptr_object) {
    double res = 0;
    if (ptr_object != NULL) {
        res = ptr_object->GetData();
    }
    return res;
}

const char* CallGetError(s21::Calculation * ptr_object) {
    return s21::ConvertErrorString(ptr_object);
}

const char* CallGetInput(s21::Calculation* ptr_object) {
    return s21::ConvertInputString(ptr_object);
}
